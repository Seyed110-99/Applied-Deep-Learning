...
def polynomial_features_matrix(x, M):
    ...
    #     # Compute the polynomial features for the i-th sample
    #     feature = torch.prod(torch.pow(x[i], multi_indices), dim=1)
...
def logistic_fun(w, M, x):
    ...
    """
    # Compute the polynomial features
    features = polynomial_features_matrix(x, M)

    # Compute the output
    y = torch.sigmoid(torch.matmul(features, w))
...
def  fit_logistic_sgd(x, y_valid, M,  optimizer = "CE", lr = 0.01, mini_batch_size =16, max_epochs=100):
    """ Fit a logistic reggression model using SGD.
    Args:
        x: torch.Tensor of shape (N, D), the input data.
        y_valid: torch.Tensor of shape (N,), the target values.
        M: int, the maximum degree.
        optimizer: str, the loss function to use. Either "CE" for cross-entropy or "RMS" for root mean square.
    ...
    if optimizer == "CE":
        ...
        raise ValueError("The optimizer must be either 'CE' or 'RMS'.")
    ...
    for epoch in range(max_epochs):
       ...
       for i in range(0, N, mini_batch_size):
           ...
           # Compute the output
           y_pred = logistic_fun(weights, M, x_mini_batch)
